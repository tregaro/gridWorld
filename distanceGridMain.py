...
class GridLayer(cocos.layer.Layer):
    ...
    def update_path(self):
        ...
        if g_player_size % 2 != 0:
            path = [(x + g_grid_size / 2.0,
                     y + g_grid_size / 2.0) for x, y in path]
    ...
    def world_to_aligned_world(world_pos):
        if g_player_size % 2 == 0:
            return tuple(round(x / g_grid_size) * g_grid_size for x in world_pos)
        else:
            return tuple(g_grid_size * 0.5 + int(x // g_grid_size) * g_grid_size for x in world_pos)
    ...
    def set_start_pos(self, position):
        if self.start_square is None:
            self.start_square = cocos.sprite.Sprite(
                image='assets/circle_line.png',
                scale=g_player_size / 2.0
            )
    ...
    def set_end_pos(self, position):
        if self.end_square is None:
            self.end_square = cocos.sprite.Sprite(
                image='assets/circle_line.png',
                scale=g_player_size / 2.0,
                color=(128, 128, 128)
            )
